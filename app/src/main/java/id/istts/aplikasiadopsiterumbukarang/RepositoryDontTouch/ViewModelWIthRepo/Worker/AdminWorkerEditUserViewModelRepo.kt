...
class AdminWorkerEditUserViewModelRepo(private val apiService: ApiService?=null,private val repository: RepositoryWorker?= RepositoryWorker(),private val context: Context?=null): ViewModel() {
    ...
    fun fetchUserById(userId: String, sessionManager: SessionManager) {
        viewModelScope.launch {
            try {
                ...
                var data = repository?.selectOneWorkerHybridly(userId,token)
                _user.value = data
    ...
    fun updateUser(userId: String, updateData: Map<String, String>, sessionManager: SessionManager) {
        viewModelScope.launch {
            try {
                ...
                val repoData = repository?.updateHybridly(userId,token,updateData)
                _updateSuccess.value = true
                _user.value = repoData
